...
<#
    .SYNOPSIS 
		Snychronizes the current branch with another branch.
	.Description
		Snychronizes the current branch with another branch.
		If a commit message is provided, this command will alsow commit
		all local changes.
		
		1)  Commit everthing if $msg is specified.
...
<#
    .SYNOPSIS 
		...
		Returns $True if the merge was succefully; otherweise, $False.
	.Description
		Tries to merge the local branch with the matching remote branch.
		Returns $True if the merge was succefully; otherweise, $False.
	.EXAMPLE
